...
import { imageUpload, imageShow, URL_SERVER } from "../../ulities/imageUpload";
...
const Slide = () => {
  ...
  const initData = async () => {
    const res = await axios.get(`${URL_SERVER}/api/category/all/banner`);
    if (res.status == 200) {
  ...
  const handleUpdateImg = async () => {
    if (form.linkImg === "" || form.linkPage === "") {
      ...
      const res = await axios.put(`${URL_SERVER}/api/category/banner/${idUpdate}`, form);
      if (res.status === 200) {
  ...
  const handleUploadImg = async () => {
    if (form.linkImg === "" || form.linkPage === "") {
      ...
      const res = await axios.post(`${URL_SERVER}/api/category/banner`, form);
      if (res.status === 200) {
        toast.success("Thành Công");
        setForm({
          linkImg: "",
  ...
  const handleDelete = async (id) => {
    const res = await axios.delete(`${URL_SERVER}/api/category/banner/delete/${id}`);
    if (res.status === 200) {
  ...
  const handleEdit = async (id) => {
    const res = await axios.get(`${URL_SERVER}/api/category/banner-detail/${id}`);
    if (res.status === 200) {
      setForm({

...
const MainCategories = () => {
  ...
  const fetchData = async () => {
    const res = await axios.get(`${URL_SERVER}/api/category/all`);
    if (res.status === 200) {
  ...
  const submitForm = async (e) => {
    e.preventDefault();
    const res = await axios.post(`${URL_SERVER}/api/category`, {
  ...
  const updateForm = async (e) => {
    e.preventDefault();
    const res = await axios.put(`${URL_SERVER}/api/category/${idEdit}`, {
      name: formCategory.name,
      isShow: formCategory.isShow,
      description: formCategory.description,
    });
  ...
  const handleChangeShowItem = async (id, showItem) => {
    const res = await axios.put(`${URL_SERVER}/api/category/status/${id}`, {
      isShow: !showItem,
    });
    if (res.status === 200) {
  ...
  const deleteItem = async (id) => {
    const res = await axios.delete(`${URL_SERVER}/api/category/${id}`);
    if (res.status === 200) {
  ...
  const handleEditItem = async (id) => {
    const res = await axios.get(`${URL_SERVER}/api/category/${id}`);
    if (res.status === 200) {
      setFormCategory({
        name: res.data.name,
